...
from lstm_download_jd_data_test1 import Data
...
class RNN:
    ...
    def __init__(self, vocab_size=4, word_dimension=32, class_num=2, unit_num=32, layer_num=4):
    ...
    def load_model(self):
        ...
        try:
            tf.train.Saver().restore(self.sess, "model/lstm_example/model.sd")
        except Exception, ex:
            print ex
    ...
    def save_model(self):
        ...
        saver.save(self.sess, "model/lstm_example/model.sd")
        tf.train.write_graph(self.sess.graph_def, "model/lstm_example", "model.proto", as_text=False)
        tf.train.write_graph(self.sess.graph_def, "model/lstm_example", "model.txt", as_text=True)

    def train_writer(self):
        return tf.summary.FileWriter('../log/lstm_train_test1', self.sess.graph)

    def train_min_data(self, min_inputs_set, min_labels_set, keep_prob=0.6, learning_rate=0.08):
        loss_total = 0
    ...
    def test_min_data(self, min_inputs_set, min_labels_set):
        ...
        right_count = 0
        with open("../../log/error.list", "w") as error_output:
            for key in min_labels_set:
                ...
                for index, pair in enumerate(zip(target_id, predict_id)):
                    ...
                    if t == p:
                        ...
                        error_output.write("%s,%d-->%d\n" % (data.show_sentence(input_data[index]), t, p))
...
def train_model():
    global data
    data = Data.Data("../data/download_comment.csv")
